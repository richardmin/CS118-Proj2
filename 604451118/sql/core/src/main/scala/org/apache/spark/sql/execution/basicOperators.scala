...
case class PartitionProject(projectList: Seq[Expression], child: SparkPlan) extends UnaryNode {
  ...
  def generateIterator(input: Iterator[Row]): Iterator[Row] = {
    ...
    //Implemented for Part 4
    
    new Iterator[Row] {
      var dp: DiskPartition = null
      val d_it = DiskHashedRelation(input, keyGenerator).getIterator()
      var r_it: Iterator[Row] = null

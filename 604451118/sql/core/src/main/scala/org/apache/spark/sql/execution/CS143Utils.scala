...
object CS143Utils {
  ...
  /**
   * Returns a Scala array that contains the bytes representing a Row.
   *
   * @param data the row we're converting
   * @return an array of bytes
   */
  ...
  def getUdfFromExpressions(expressions: Seq[Expression]): ScalaUdf = {
    ...
    //internal function
    def recursiveUDFTraversal(tmp_expressions: Seq[Expression]): ScalaUdf = {
      tmp_expressions match {
        case Nil            => null //if it's nil
        case hd :: tail => hd match 
        { //scala pattern matching: https://kerflyn.wordpress.com/2011/02/14/playing-with-scalas-pattern-matching/
          case found: ScalaUdf =>    //basically you get the first node, if it's found you're good, otherwise if we find a scala we're good (typecheck)
          {
           found //found is the last node
          }
          case otherwise => 
          {
            recursiveUDFTraversal(tail)}
        }
      }
    }

    recursiveUDFTraversal(expressions.reverse)
  }
        ...
        def next() = {
          ...
          if(input.hasNext)
          {
            ...
            val udf: Row = {
              ...
              {
                val v: Row = udfProject.apply(row)
                cache.put(key, v)
                v
              }
            }
